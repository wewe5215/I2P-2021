//
//  main.c
//  I2P-II-mini_project1
//
//  Created by 朱季葳 on 2021/11/12.
//
...
void semantic_check(AST *now) {
    ...
    if(now->kind == POSTINC || now->kind == POSTDEC || now->kind == PREINC || now->kind == PREDEC){
        AST *tmp = now->mid;
        while (tmp->kind == LPAR) {
            tmp = tmp->mid;
        }
        if(tmp->kind != IDENTIFIER){
            ...
            err("Lvalue/Rvalue is required as left/right operand of INC/DEC.");
