...
const std::list<int>PlayScene::code={ALLEGRO_KEY_UP,ALLEGRO_KEY_UP,ALLEGRO_KEY_DOWN,ALLEGRO_KEY_DOWN,ALLEGRO_KEY_LEFT,ALLEGRO_KEY_RIGHT,ALLEGRO_KEY_ENTER};
...
void PlayScene::Update(float deltaTime) {
	...
	for (int i = 0; i < SpeedMult; i++) {
        ...
        for(int j = 0 ; j < std::get<2>(current) ; j++){
            ...
            switch (std::get<0>(current)) {
                ...
                case 4:
                    EnemyGroup->AddNewObject(enemy = new Enemy3(SpawnCoordinate.x, SpawnCoordinate.y));
                    break;
                default:
					continue;
			}
    ...
    //to see where will the turret be placed
	if (preview) {
...
void PlayScene::ConstructUI() {
    ...
    //button 3
    btn = new TurretButton("play/floor.png", "play/dirt.png",
        Engine::Sprite("play/turret-6.png", 420, BlockSize*MapHeight, 0, 0, 0, 0)
        , 410, 128*MapHeight, Turret3::Price);
    btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, 2));
    UIGroup->AddNewControlObject(btn);
    //button 4
    btn = new TurretButton("play/floor.png", "play/dirt.png",
        Engine::Sprite("play/turret-4.png", 540, BlockSize*MapHeight, 0, 0, 0, 0)
        , 530, 128*MapHeight, Turret3::Price);
    btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, 3));
    UIGroup->AddNewControlObject(btn);
    //button 5
    btn = new TurretButton("play/floor.png", "play/dirt.png",
        Engine::Sprite("play/turret-3.png", 660, BlockSize*MapHeight, 0, 0, 0, 0)
        , 650, 128*MapHeight, Cherry_Turret::Price);
    btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, 4));
    UIGroup->AddNewControlObject(btn);
    //button 6
    btn = new TurretButton("play/floor.png", "play/dirt.png",
        Engine::Sprite("play/turret-5.png", 780, BlockSize*MapHeight, 0, 0, 0, 0)
        , 770, 128*MapHeight, Clear_Turret::Price);
...
void PlayScene::UIBtnClicked(int id) {
    ...
    else if(id == 2 &&money >= Turret3::Price)
        preview = new Turret3(0,0);
    else if(id==3&&money>=PotatoTurret::Price)
        preview = new PotatoTurret(0,0);
    else if(id == 4&&money>=Cherry_Turret::Price)
        preview =new Cherry_Turret(0,0);
    else if (id==5&&money>=Clear_Turret::Price)
    {
        //printf("%d\n",Clear_Turret::Price);
        preview=new Clear_Turret(0,0);
    }
	if (!preview)
		return;
	preview->Position = Engine::GameEngine::GetInstance().GetMousePosition();
